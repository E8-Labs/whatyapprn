...
import { Modal, View, Text, StyleSheet, TouchableOpacity, Image, Alert,ActivityIndicator } from 'react-native';
...
const AddCardScreen = ({ close }) => {
  ...
  const handleAddCard = async () => {
    ...
    try {
      setLoading(true);
      const { token, error } = await stripe.createToken({ type: 'Card', ...cardDetails });
      ...
      <Modal visible={true} transparent={true} animationType="fade">
        <View style={styles.container}>
          <View style={styles.modalContent}>
            ...
            {/* Submit Button */}
            {
              loading ? <ActivityIndicator size="large" color={Colors.orangeColor} /> : (
                <TouchableOpacity
