                    ...
                    renderItem={({ item }) => (
                        <>
                            <TouchableOpacity onPress={() => handleOnPress(item)} >
                                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: screenWidth - 60, alignSelf: 'center', paddingTop: 20, }}>
                                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                                        <Image source={item.Business.profile_image ? { uri: item.Business.profile_image } : placeholder}
                                            // onLoadStart={() => {
                                            //     setLoadImage(true)
                                            // }}
                                            // onLoadEnd={() => {
                                            ...
                                            // }}
                                            style={{
                                                resizeMode: 'cover', height: 49 / 930 * screenHeight, width: 49 / 930 * screenHeight,
                                        ...
                                        <View style={{ flexDirection: 'column', alignItems: 'flex-start', gap: 3 }}>
                                            <Text style={{ fontSize: 17, fontFamily: CustomFonts.meduim, color: item.unread ? "#000" : Colors.unreadColor }}>
                                                {item.Business.name}
                                            </Text>
                                            <Text numberOfLines={1} lineBreakMode='tail' style={{
                                                fontSize: 12, fontFamily: CustomFonts.regular, width: 230 / 430 * screenWidth, color: item.unread ? "#000" : Colors.unreadColor
                                            }}>
                                    ...
                                    <View style={{ flexDirection: 'column', alignItems: 'flex-end', gap: 3 }}>
                                        <Text style={{ fontSize: 12, fontFamily: CustomFonts.regular, color: item.unread ? "#000" : Colors.lightBlack }}>
                                           
                                            {moment(item.createdAt).format("h:mm A")}
                                        </Text>
                                        {
                                            item.unread ? (
                                                <View style={{ backgroundColor: Colors.orangeColor, height: 20, borderRadius: 50, width: 20, alignItems: 'center', justifyContent: 'center' }}>
